...
<li class="am-comment am-comment-primary">
  ...
  <div class="am-comment-main">
	...
	<c:forEach items="${pro.saysreply}" var="rep">
		<li class="am-comment am-comment-flip"><a href="#link-to-user-home"><img title="${rep.userid.userimg}" src="images/${rep.userid.userimg}" alt="" class="am-comment-avatar" width="48" height="48"></a><div class="am-comment-main"><header class="am-comment-hd"><div class="am-comment-meta"><a href="#link-to-user" class="am-comment-author">${rep.userid.usernickname }</a> 回复于<span> ${rep.replydate}</span></div></header><div class="am-comment-bd"><p><a  href="user/myhome?userid=${pro.userid.usernickname}" target="view_window" >@${pro.userid.usernickname}</a> ${rep.replycontent }</p></div></div></li>
	</c:forEach>
</c:forEach>
  <!--  <ul class="am-list">
<c:forEach items="${page1.result}" var="pro">
